...
void insert(int key, int value) {
    ...
    newNode->value = value;
    newNode->next = NULL;
    if (hashTable[index] == NULL) { // Si el índice de la tabla está vacío
        hashTable[index] = newNode; // Asigna el nuevo nodo como el primer elemento en ese índice
    } else { // Si el índice de la tabla ya tiene elementos
        HashNode* currentNode = hashTable[index]; // Obtiene el primer nodo en ese índice
        while (currentNode->next != NULL) { // Avanza hasta el último nodo en la lista
...
int* search(int key) {
    ...
    while (node != NULL) { // Recorre la lista de nodos en ese índice
        if (node->key == key) { // Si el nodo tiene la key buscada
...
void destroy() {
    for (int i = 0; i < TABLE_SIZE; i++) { // Recorre todos los índices de la tabla
...
void saveHashTable(char* filename) {
    ...
    if (fp != NULL) { // Si el archivo se abrió correctamente
        for (int i = 0; i < TABLE_SIZE; i++) { // Recorre todos los índices de la tabla
            HashNode* node = hashTable[i]; // Obtiene el primer nodo en ese índice
            while (node != NULL) { // Recorre la lista de nodos en ese índice
...
void loadHashTable(char* filename) {
    ...
    if (fp != NULL) { // Si el archivo se abrió correctamente
        ...
        int key, value;
        while (fread(&key, sizeof(int), 1, fp) && fread(&value, sizeof(int), 1, fp)) { // Lee una key y un valor del archivo
